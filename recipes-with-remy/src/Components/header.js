...
function Header() {
  ...
  const getNavLinkStyle = (path) => {
    // Check if the current hash includes the specific path (adjust the path to include the hash '#')
    return window.location.hash === `#${path}` ? { fontSize: '1.5rem', color: 'red' } : { fontSize: '1.5rem' };
  };
